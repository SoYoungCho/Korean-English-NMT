...
class EncoderRNN(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size, dropout_p=0,
                 n_layers=1, bidirectional=True, rnn_cell='gru', embedding=None):
        ...
        # 위 인자들 1번째는 인풋 사이즈 2번째는 아웃풋 사이즈라고 생각해주면 됨.
        # 인코더 디코더 사이즈가 달라진다는건 ? 위 n_layers 가 달라진다고 생각하면 됨 보통 인코더를 깊게, 디코더를 얇게.
    ...
    def forward(self, input_var, input_lengths=None):
        ...
        output, hidden = self.rnn(embedded) # rnn(인풋) forward 이 rnn에 이미 구현이 되어 있음 (물론 또 따로 호출해도 됨)
